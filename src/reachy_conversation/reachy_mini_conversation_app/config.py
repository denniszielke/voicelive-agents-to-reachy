...
def _resolve_default_profiles_directory() -> Path:
    ...
    # This port lives at ``src/reachy_conversation/reachy_mini_conversation_app``
    # and ships its own ``profiles/`` directory next to the package. Prefer it so
    # the app is self-contained inside the VoiceLive repo.
    package_profiles = Path(__file__).resolve().parents[1] / "profiles"
    if package_profiles.is_dir():
...
# --- Azure VoiceLive backend ------------------------------------------------
# This port replaces the Hugging Face realtime WebSocket with Azure VoiceLive
# bound to a Foundry hosted agent (the orchestrator). Speech I/O (STT, turn
# detection, TTS) is handled by VoiceLive; the conversation is driven by the
# hosted agent.
VOICELIVE_BACKEND = "voicelive"

# Curated Azure neural HD voices exposed to the UI / voice-change tool. These
# replace the Hugging Face speaker catalog.
VOICELIVE_AVAILABLE_VOICES: list[str] = [
...
VOICELIVE_DEFAULT_VOICE = VOICELIVE_AVAILABLE_VOICES[0]

# Environment variable names (shared with the other VoiceLive front-ends in this repo).
AZURE_VOICELIVE_ENDPOINT_ENV = "AZURE_VOICELIVE_ENDPOINT"
AZURE_VOICELIVE_MODEL_ENV = "AZURE_VOICELIVE_MODEL"
AZURE_AI_HOSTED_AGENT_NAME_ENV = "AZURE_AI_HOSTED_AGENT_NAME"
AZURE_AI_AGENT_NAME_ENV = "AZURE_AI_AGENT_NAME"
AZURE_AI_PROJECT_NAME_ENV = "AZURE_AI_PROJECT_NAME"
...
class Config:
    ...
    # --- Azure VoiceLive backend settings ---
    AZURE_VOICELIVE_ENDPOINT = (os.getenv(AZURE_VOICELIVE_ENDPOINT_ENV) or "").strip()
    AZURE_VOICELIVE_MODEL = (os.getenv(AZURE_VOICELIVE_MODEL_ENV) or "").strip() or "gpt-realtime"
    # Which Foundry hosted agent VoiceLive routes turns to (default: orchestrator).
    AZURE_AI_HOSTED_AGENT_NAME = (
        os.getenv(AZURE_AI_HOSTED_AGENT_NAME_ENV)
        or os.getenv(AZURE_AI_AGENT_NAME_ENV)
        or "orchestrator-agent"
    ).strip()
    AZURE_AI_PROJECT_NAME = (os.getenv(AZURE_AI_PROJECT_NAME_ENV) or "").strip()
...
def refresh_runtime_config_from_env() -> None:
    ...
    config.AZURE_VOICELIVE_ENDPOINT = (os.getenv(AZURE_VOICELIVE_ENDPOINT_ENV) or "").strip()
    config.AZURE_VOICELIVE_MODEL = (os.getenv(AZURE_VOICELIVE_MODEL_ENV) or "").strip() or "gpt-realtime"
    config.AZURE_AI_HOSTED_AGENT_NAME = (
        os.getenv(AZURE_AI_HOSTED_AGENT_NAME_ENV)
        or os.getenv(AZURE_AI_AGENT_NAME_ENV)
        or "orchestrator-agent"
    ).strip()
    config.AZURE_AI_PROJECT_NAME = (os.getenv(AZURE_AI_PROJECT_NAME_ENV) or "").strip()


def get_available_voices() -> list[str]:
    """Return the curated Azure VoiceLive voice list."""
    return list(VOICELIVE_AVAILABLE_VOICES)


def get_default_voice() -> str:
    """Return the default Azure VoiceLive voice."""
    return VOICELIVE_DEFAULT_VOICE
