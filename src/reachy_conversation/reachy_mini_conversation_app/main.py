...
def run(
    ...
    logger.info(
        "Configured Azure VoiceLive backend bound to hosted agent %r (project %r)",
        config.AZURE_AI_HOSTED_AGENT_NAME,
        config.AZURE_AI_PROJECT_NAME or "<default>",
    )
    ...
    def build_handler(startup_voice: Optional[str] = None) -> ConversationHandler:
        """Build the Azure VoiceLive realtime handler bound to the hosted agent."""
        from reachy_mini_conversation_app.voicelive_realtime import VoiceLiveRealtimeHandler

        logger.info(
            "Using Azure VoiceLive realtime handler (agent=%r)",
            config.AZURE_AI_HOSTED_AGENT_NAME,
        )
        return VoiceLiveRealtimeHandler(
            deps,
            instance_path=instance_path,
